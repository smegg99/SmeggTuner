...
export function columnGroups(count: number): ColumnGroup[] {
  ...
  for (const reed of reedIndexes(count)) {
    if (reed > 0) {
      const low = reed - 1
      out.push({ kind: 'beat', key: `beat-${low}`, pair: `${low + 1}-${reed + 1}`, low, high: reed })
    }
...
export function reedGroups(count: number): ReedGroup[] {
  return reedIndexes(count).map(reed => ({ kind: 'reed', key: `reed-${reed}`, reed }))
}
